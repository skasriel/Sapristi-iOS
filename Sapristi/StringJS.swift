...
extension String {
    ...
    public func startsWith(string: String) -> Bool {
        var index: Int? = self.indexOf(string);
        return (index == 0);
    }
    ...
    public func indexOf(string: String, startFrom: Int = 0) -> Int? {
        ...
        var index = (self as NSString).rangeOfString(string).location
        if index == NSNotFound {
            return nil
        }
    ...
    public func substr(var index: Int) -> String {
        ...
        return (self as NSString).substringWithRange(NSMakeRange(index, self.length - index))
    }
